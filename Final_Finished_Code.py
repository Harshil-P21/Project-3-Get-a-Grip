...
#takes ID number as parameter then returns location of dropoff based on ID
...
def move_end_effector(ID): 
    ...
    #move box to dropoff location
        ...
        location = autobin_location(ID)#set  ID location as a list
...
#close autoclave depending on ID number
...
def spawn_cage():
    ...
    #check if the random has already been used by comparing to list
...
#main function
def main():
    #number if cycles program has run through
    cycle = 0

    #no cage is being moved
    running = False

    #when no cage is being mvoed
    while running == False:
        #when the program is on its 6th cycle
        if cycle == 6:
            #exit the program
            sys.exit()

        #add 1 to the number of cycles
        cycle += 1
        ...
        while running == True:
            ...
            #if the function want to pick up the box close/open the gripper
            if move_value == 'pickUpBox':                
                ...
                while running == True:
                    ...
                    if move_end_effector(ID) == 'goHome':
                        ...
                        #program is no longer moving box
                        running = False
